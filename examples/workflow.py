...
from dotspy import Edge, Graph, GraphStyle, Node, NodeStyle, Subgraph
...
def generate_graph():
    ...
    with Graph("Workflow", rankdir="TD") as g:
        ...
        # Global node style
        with NodeStyle(
            shape="rect", style="filled", fillcolor="white", fontname="Helvetica"
        ):
            ...
            with Subgraph(
                ...
                gen = Node(
                    "gen",
                    label="Generate Sequences\n(ActorRolloutWG.generate_sequences)",
                    fillcolor="#fff59d",
                )
                compute_log_probs = Node(
                    "compute_log_probs",
                    label="Compute Old Log Probs\n(ActorWG)",
                    fillcolor="#fff59d",
                )
                compute_ref = Node(
                    "compute_ref",
                    label="Compute Ref Log Probs\n(RefPolicyWG)",
                    fillcolor="#fff59d",
                )
                compute_values = Node(
                    "compute_values",
            ...
            with Subgraph(
                ...
                calc_adv = Node(
                    "calc_adv",
                    label="Compute Advantages (GAE)\n(Driver)",
                    fillcolor="#b3e5fc",
                )
                make_batch = Node(
                    "make_batch",
                    label="Construct Training Batch\n(Driver)",
                    fillcolor="#b3e5fc",
                )
            ...
            with Subgraph(
                ...
                update_actor = Node(
                    "update_actor",
                    label="Update Actor\n(ActorWG.update_actor)",
                    fillcolor="#ffab91",
                )
                update_critic = Node(
                    "update_critic",
                    label="Update Critic\n(CriticWG.update_critic)",
                    fillcolor="#ffab91",
                )
